...
class TransactionDetail extends PureComponent {
	...
	openBlock = (block): void => {
		...
		if (selectedCrypto === "auroracoin") url = `http://insight.auroracoin.is/block/${block}`;
	...
	openAddress = (address = ""): void => {
		...
		if (selectedCrypto === "auroracoin") url = `http://insight.auroracoin.is/address/${address}`;
	...
	openMessage = (tx = ""): void => {
		...
		switch (selectedCrypto) {
			...
			case "canadaecoin":
				...
				break;	
			case "auroracoin":
				url = `http://insight.auroracoin.is/tx/${tx}`;
				break;	
